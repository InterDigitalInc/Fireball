...
options = DashOptions(
    [
        ...
        ('dropOut',        'Drop Out Probability for NN algorithm.(Default: 0)',                ['float', 'min:0.0', 'max:0.9', 'default:0.3']),
...
if __name__ == '__main__':
    ...
    myPrint('\nPreparing Even/Odd datasets ... ')
    
    if options.get('useTest'):
        trainDs, testDs = EvenOddDSet.makeDatasets('train,test', batchSize=options.get('batchSize'))
        validDs = None
    else:
        trainDs, testDs, validDs = EvenOddDSet.makeDatasets('train,test,valid', batchSize=options.get('batchSize'))
    
    
    EvenOddDSet.printStats(trainDs, testDs, validDs)

    model = Model(name='EvenOdd',
                  layersInfo = 'IMG_S28_D1,CONV_K5_O6_Ps:ReLU:MP_K2,CONV_K5_O16_Pv:ReLU:MP_K2,FC_O120:ReLU,FC_O84:ReLU,FC_O1:None,CLASS_C2',
