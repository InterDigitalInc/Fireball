...
options = DashOptions(
    [
     ...
     ('coreml',       'Quantize for coreml export. (Reuse empty clusters, quantize weights only)',  ['bool']),
    ])
...
if __name__ == '__main__':
    ...
    Model.quantizeModel(options.get('in'), options.get('out'),
                        ...
                        reuseEmptyClusters=options.get('coreml'), weightsOnly=options.get('coreml'), numWorkers=0)
